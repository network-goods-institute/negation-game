...
export const useConnectionHandlers = ({
  ...
  const beginConnectFromNode = useCallback((id: string, cursor?: { x: number; y: number }) => {
  ...
  const beginConnectFromEdge = useCallback((edgeId: string, cursor?: { x: number; y: number }) => {
    ...
    if (edge) {
      ...
      const anchorNode: Node = { id: anchorId, type: 'edge_anchor', position, data: { parentEdgeId: edgeId } } as Node;
      setNodes((nds) => nds.some(n => n.id === anchorId) ? nds : [...nds, anchorNode]);
    }
  }, [edges, connectAnchorRef, setConnectAnchorId, setConnectCursor, setNodes, getEdgeMidpoint]);

  const completeConnectToNode = useCallback((nodeId: string) => {
    ...
    if (anchorId.startsWith('anchor:')) {
      ...
      if (!anchorNodeExists) {
        ...
        const anchorNode: Node = { id: anchorIdForEdge, type: 'edge_anchor', position: midpoint, data: { parentEdgeId: edgeId } } as Node;
        setNodes((nds) => nds.some(n => n.id === anchorIdForEdge) ? nds : [...nds, anchorNode]);
        if (yNodesMap && ydoc && canWrite) {
          ydoc.transact(() => { if (!yNodesMap.has(anchorIdForEdge)) yNodesMap.set(anchorIdForEdge, anchorNode); }, localOrigin);
        }
      }
      const newObjEdge: Edge = { id: generateEdgeId(), type: 'objection', source: nodeId, target: anchorIdForEdge } as Edge;
      setEdges((eds) => eds.some(e => e.id === newObjEdge.id) ? eds : [...eds, newObjEdge]);
      if (yEdgesMap && ydoc && canWrite) {
        ydoc.transact(() => { if (!yEdgesMap.has(newObjEdge.id)) yEdgesMap.set(newObjEdge.id, newObjEdge); }, localOrigin);
      }
      setConnectAnchorId(null);
      connectAnchorRef.current = null;
      setConnectCursor(null);
    ...
    if (!exists) {
      setEdges((eds) => (eds.some(e => e.id === id) ? eds : [...eds, edge]));
    }
    if (yEdgesMap && ydoc && canWrite) {
      ydoc.transact(() => { if (!yEdgesMap.has(id)) yEdgesMap.set(id, edge); }, localOrigin);
    }
    setConnectAnchorId(null);
    connectAnchorRef.current = null;
    setConnectCursor(null);
  }, [connectMode, canWrite, connectAnchorId, connectAnchorRef, nodes, edges, yNodesMap, yEdgesMap, ydoc, localOrigin, setNodes, setEdges, setConnectAnchorId, setConnectCursor, setConnectMode, isLockedForMe, getLockOwner, getEdgeMidpoint]);
  ...
  const completeConnectToEdge = useCallback((edgeId: string, midX?: number, midY?: number) => {
    if (!connectMode) return;
    ...
    if (originNode) {
      ...
      if (!anchorExists) {
        ...
        const anchorNode: Node = { id: anchorId, type: 'edge_anchor', position: midpoint, data: { parentEdgeId: edgeId } } as Node;
        setNodes((nds) => nds.some(n => n.id === anchorId) ? nds : [...nds, anchorNode]);
        if (yNodesMap && ydoc && canWrite) {
          ydoc.transact(() => { if (!yNodesMap.has(anchorId)) yNodesMap.set(anchorId, anchorNode); }, localOrigin);
        }
      }
      const newEdge: Edge = { id: generateEdgeId(), type: 'objection', source: originNode.id, target: `anchor:${edgeId}` } as Edge;
      setEdges((eds) => eds.some(e => e.id === newEdge.id) ? eds : [...eds, newEdge]);
      if (yEdgesMap && ydoc && canWrite) {
        ydoc.transact(() => { if (!yEdgesMap.has(newEdge.id)) yEdgesMap.set(newEdge.id, newEdge); }, localOrigin);
      }
    }
    setConnectAnchorId(null);
    connectAnchorRef.current = null;
    setConnectCursor(null);
  }, [connectMode, connectAnchorRef, nodes, yNodesMap, yEdgesMap, ydoc, canWrite, localOrigin, setNodes, setEdges, setConnectAnchorId, setConnectCursor, getEdgeMidpoint]);
