 ...
 */
const getNodeDimension = (node: Node, dimension: 'width' | 'height'): number => {
  const measured = node.measured as { width?: number; height?: number } | undefined;
  const style = node.style as { width?: number; height?: number } | undefined;

  if (typeof (node as any)[dimension] === 'number') return (node as any)[dimension];
...
export const useNodeHelpers = ({ nodes, edges }: UseNodeHelpersProps) => {
  ...
  const getNodeCenter = useCallback((nodeId: string): { x: number; y: number } | null => {
    ...
    const abs = (node as any).positionAbsolute || node.position || { x: 0, y: 0 };
   ...
   */
  const getEdgeMidpoint = useCallback((edgeId: string): { x: number; y: number } | null => {
