...
export const initUserAction = async ({
  ...
  logger.warn("[initUserAction] Initializing user with username:", username);

  try {
    const userId = await getUserId();
    logger.warn("[initUserAction] Got user ID:", userId);

    if (!userId) {
      logger.error("[initUserAction] Authentication error: No user ID found");
    ...
    logger.warn("[initUserAction] User creation result:", result);
    ...
    logger.error("[initUserAction] Error creating user:", error);
    throw error;
  }
};
