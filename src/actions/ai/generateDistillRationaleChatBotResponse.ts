...
*   **Ensure the refined essay remains grounded in the original rationale's content, especially the endorsed points, and does not introduce new topics or points not derivable from the source rationale.**`;
    ...
    try {
      ...
      if (selectedRationaleId && fetchedRationaleData) {
        ...
        *   **No Bracket Tags:** Do NOT use \`[Point:ID]\` or \`[Rationale:ID]\` tags in your essay response. Weave the arguments from the endorsed points (using their content from the graph nodes) into the narrative naturally.`;
        ...
        suggestionRule = `4.  **Suggesting New Points:** When suggesting a new, independent point, use \`[Suggest Point]>\` on **its own line**, followed by the suggested point text on the next line(s). Render this as a distinct block. **The suggested point text MUST be less than 200 characters.**
    *   Example:
        [Suggest Point]>\n        We should consider the long-term maintenance costs.\n\n5.  **Suggesting Negations (Counterpoints & Objections):** When suggesting a negation (either a direct counterpoint or an objection) for a specific **existing point** (e.g., Point 123 provided in the CONTEXT), place the \`[Suggest Negation For:123]>\` or \`[Suggest Objection For:123]>\` tag **immediately after discussing Point 123**, on a **new line**, potentially indented or formatted like a sub-item. Follow the tag immediately with the negation/objection text. \`ID\` **must be the numeric ID of an EXISTING Point** provided in the CONTEXT section. **DO NOT suggest negations or objections for Rationales (string IDs) or Discourse Posts (numeric IDs). Do NOT invent Point IDs.** **The suggested negation/objection text MUST be less than 200 characters.**
    ...
    *   Example (after discussing Point 456):\n        ... discussion referencing Point 456 (Source: Endorsed Point ID:456).\n        - [Suggest Objection For:456]> This argument is irrelevant to the core issue of project scalability.`;
