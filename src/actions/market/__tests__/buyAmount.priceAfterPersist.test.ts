...
describe("price_after_scaled persistence", () => {
  ...
  it("persists price_after_scaled on the main trade row", async () => {
    ...
    const main = flat.find(
      (r) => r && r.securityId === "p-a" && r.deltaScaled && r.deltaScaled !== "0"
    );
    expect(main).toBeTruthy();
    expect(typeof main.priceAfterScaled).toBe("string");
  });
});
