...
export async function buyAmount(docId: string, securityId: string, spendScaled: string) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    await tx.execute(sql`INSERT INTO market_state (doc_id, version, updated_at) VALUES (${canonicalId}, 0, NOW()) ON CONFLICT (doc_id) DO NOTHING`);
    await tx.execute(sql`SELECT 1 FROM market_state WHERE doc_id = ${canonicalId} FOR UPDATE`);
    const rows = await tx
    ...
    let mm = createMarketMaker(structure, defaultB, securities, { enumerationCap: 1 << 18 });
    ...
    if (!totals.has(normalized)) {
      ...
      if (looksLikeNode || looksLikeEdge) {
        const edgeTriples = (structure.edges as any[]).map((e: any) => [e.name, e.from, e.to]) as [string, string, string][];
        ...
        try {
          logger.warn('[market] Unknown security on buyAmount', {
            docId,
            userId,
    ...
    try { priceAfter = mm.getPrices()?.[normalized]; } catch {}

    const existing = await tx
      .select({ id: marketHoldingsTable.id, amountScaled: marketHoldingsTable.amountScaled })
      .from(marketHoldingsTable)
      .where(
        and(
    ...
    const newAmount = (existing[0] ? BigInt(existing[0].amountScaled) : 0n) + shares;
    if (existing[0]) {
      await tx
    ...
    await tx
      ...
      .set({ version: sql`${marketStateTable.version} + 1`, updatedAt: new Date() })
