...
export const createInversePair = (
  ...
  return (pointNodeId: string) => {
    ...
    const createInversePairOperation = () => {
      ...
      if (pointNode?.data?.inverseGenerated) {
        ...
        if (yNodesMap && ydoc && canWrite) {
          ...
          }, "inverse-pair-operation"); // Use special origin not tracked by undo manager
        }
      ...
      const gapWidth = 25; // Match GroupNode gap between children
      const containerWidth =
        padding + nodeWidth + gapWidth + nodeWidth + padding; // left + leftNode + gap + rightNode + right
      const containerHeight = nodeHeight + padding * 8;
      ...
      // Sync to Yjs with untracked origin to avoid undo/redo
      if (yNodesMap && ydoc && canWrite) {
        ydoc.transact(() => {
          // Add group first
          yNodesMap.set(groupId, groupNode);
      ...
      // Don't update local state - let Yjs observer handle it to avoid race conditions

      const safeHas = (id: string) =>
        typeof yNodesMap?.has === "function" ? yNodesMap.has(id) : true;

      // Add timeout to AI generation (7 seconds)
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error("AI generation timeout")), 7000);
      });

      Promise.race([generateInversePoint(originalContent), timeoutPromise])
        .then((aiContent) => {
          if (
            !safeHas(groupId) ||
            !safeHas(inverseId) ||
        ...
        .catch(() => {
          // Always try to set fallback content, even if some nodes are missing
          const fallbackText = `Not ${originalContent}`;
            ...
            ydoc.transact(() => {
              ...
              if (t) {
                ...
                if (curr !== fallbackText) {
                  // eslint-disable-next-line drizzle/enforce-delete-with-where
                  if (curr && curr.length) t.delete(0, curr.length);
                  if (fallbackText) t.insert(0, fallbackText);
                }
              }
              const oBase = yNodesMap.get(pointNodeId);
              if (oBase) {
