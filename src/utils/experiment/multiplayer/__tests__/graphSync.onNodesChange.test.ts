...
class YMapLike<V> {
  ...
  set(id: string, v: V) { this.setCalls.push(id); this.map.set(id, v); }
...
describe("graphSync.onNodesChange writes only changed positions/dims", () => {
  it("updates only nodes whose position changed", () => {
    ...
    let nodes = [
      ...
      { id: "c", type: "point", position: { x: 21, y: 20 }, data: {} },
    ];

    const setNodes = (updater: any) => { nodes = updater(nodes); return nodes; };
    ...
    expect(last).toBe("c");
    const counts = yNodes.setCalls.reduce((m: any, k) => ((m[k] = (m[k] || 0) + 1), m), {} as any);
