...
import { usePriceHistory, PricePoint } from "@/hooks/market/usePriceHistory";
...
export const InlinePriceHistory: React.FC<Props> = ({
  ...
  if (loading) {
    return (
      <div className={`w-full min-w-0 overflow-hidden ${className}`}>
        <div
          ref={boxRef}
          className="w-full min-w-0 pointer-events-none select-none overflow-hidden"
        >
          <div
            className={`w-full min-w-0 box-border overflow-hidden rounded-md subpixel-antialiased font-sans ${
              compact ? "" : "px-2 py-1.5"
            } ${
              compact
                ? ""
          ...
          >
            <div
              className={`text-[14px] font-semibold ${compact ? "mb-0.5" : "mb-1"} ${
                variant === "objection" ? "text-amber-700" : "text-emerald-600"
              }`}
            >
  ...
  const displaySeries: PricePoint[] = (() => {
    ...
    try {
      ...
      if (
        ...
        Math.abs(lastPrice - Number(currentPrice)) > 1e-9
      ) {
        series.push({
  ...
  if (hasValidTimes) {
    const validPairs = displaySeries
      .map((p, i) => ({ t: times[i], price: prices[i] }))
  ...
  return (
    <div className={`w-full min-w-0 overflow-hidden ${className}`}>
      <div
        ref={boxRef}
        className="w-full min-w-0 select-none overflow-hidden"
      ...
      >
        ...
        >
          {!hideHeader && (
            <div
              className={`w-full min-w-0 overflow-hidden text-[14px] font-semibold ${
                compact ? "mb-0.5" : "mb-1"
              } ${
                variant === "objection" ? "text-amber-700" : "text-emerald-600"
              }`}
        ...
        createPortal(
          ...
          >
            <div className="font-semibold text-gray-900">
              {(tooltipData.price * 100).toFixed(1)}%
            </div>
            ...
            {(() => {
              const tradeInfo = formatTradeInfo(
                tooltipData.deltaScaled,
                tooltipData.costScaled
              );
              const securityType = getSecurityType();
              if (!tradeInfo) {
                return (
                  <div className="text-[10px] border-t border-gray-200 pt-1">
                    <div className="text-gray-500 italic">Price updated</div>
              ...
              return (
                <div className="text-[10px] border-t border-gray-200 pt-1 space-y-0.5">
                  ...
                  <div className="text-gray-600">for ${tradeInfo.cost}</div>
