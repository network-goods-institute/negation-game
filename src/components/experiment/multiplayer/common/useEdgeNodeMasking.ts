...
export const useEdgeNodeMasking = (sourceNode: any, targetNode: any): NodeMaskingData => {
  return useMemo(() => {
    const srcHasFavor = sourceNode?.type === 'point' || sourceNode?.type === 'objection';
    const tgtHasFavor = targetNode?.type === 'point' || targetNode?.type === 'objection';
