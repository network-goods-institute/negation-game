...
describe("computeMindchangeStrokeWidth", () => {
  ...
  it("uses center width when mindchange is absent", () => {
    const wMin = visual.strokeWidth(0);
    const wMax = visual.strokeWidth(100);
    const center = (wMin + wMax) / 2;
    const width = computeMindchangeStrokeWidth({ visual, mindchange: undefined });
  ...
  it("uses center width when no mindchange counts", () => {
    ...
    const wMax = visual.strokeWidth(100);
    const center = (wMin + wMax) / 2;
    const width = computeMindchangeStrokeWidth({ visual, mindchange: { forward: { average: 0, count: 0 }, backward: { average: 0, count: 0 } } });
    expect(width).toBe(center);
  });

  it("uses max width at 100 average", () => {
    const width = computeMindchangeStrokeWidth({ visual, mindchange: { forward: { average: 100, count: 3 }, backward: { average: 0, count: 0 } } });
  ...
  it("applies compressed scaling between min and max", () => {
    ...
    const width = computeMindchangeStrokeWidth({ visual, mindchange: { forward: { average: 25, count: 4 }, backward: { average: 10, count: 1 } } });
