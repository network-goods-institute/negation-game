...
function computeBoundingBox(
    nodes: (PreviewAppNode | undefined)[],
    pad = 24
): BBox | null {
    if (!nodes || nodes.length === 0 || nodes.every(n => !n))
        return null;

    const validNodes = nodes.filter(n => n !== undefined) as PreviewAppNode[];
...
export const PreviewMergeNodesFrame = () => {
    const [dialogState, setDialogState] = useAtom(mergeNodesDialogAtom);
    const [isMerging, setIsMerging] = useState(false);
    const { getEdges, setEdges, setNodes } = useReactFlow<PreviewAppNode>();
    ...
    const handleMerge = async () => {
        ...
        setIsMerging(true);
        try {
            // For preview, we just merge the local nodes without database operations
