...
CREATE MATERIALIZED VIEW "public"."point_favor_history" AS (with "all_events" as ((((select "id" as point_id, "created_at" as event_time, 'point_created' as event_type from "points") union (select "point_id" as point_id, "created_at" as event_time, 'endorsement_made' as event_type from "endorsements")) union (select "older_point_id" as point_id, "created_at" as event_time, 'negation_made' as event_type from "negations")) union (select "newer_point_id" as point_id, "created_at" as event_time, 'negation_made' as event_type from "negations")) select "all_events".point_id as "point_id", "all_events".event_type as "event_type", "all_events".event_time as "event_time", 
        COALESCE((
          SELECT SUM("cred")
