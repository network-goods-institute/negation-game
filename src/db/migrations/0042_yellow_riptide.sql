...
WITH "all_events" AS (
  SELECT 
    ...
    'point_created'::text as event_type
  FROM "points"
  UNION ALL
  SELECT 
    point_id,
    created_at as event_time,
    'endorsement_made'::text as event_type
  FROM "endorsements"
  UNION ALL
  SELECT 
    older_point_id as point_id,
    created_at as event_time,
    'negation_made'::text as event_type
  FROM "negations"
  UNION ALL
  SELECT 
    newer_point_id as point_id,
    created_at as event_time,
    'negation_made'::text as event_type
  FROM "negations"
  UNION ALL
  ...
  SELECT 
    r.point_id,
    rh.created_at as event_time,
    'restake_modified'::text as event_type
  FROM "restake_history" rh
  INNER JOIN "restakes" r ON rh.restake_id = r.id
  UNION ALL
  SELECT 
    r.point_id,
    sh.created_at as event_time,
    'slash_modified'::text as event_type
  FROM "slash_history" sh
  INNER JOIN "slashes" s ON sh.slash_id = s.id
  INNER JOIN "restakes" r ON s.restake_id = r.id
  UNION ALL
  SELECT 
    id as point_id,
    NOW() as event_time,
    'favor_queried'::text as event_type
  FROM "points"
  UNION ALL
  SELECT 
    d.point_id,
    dh.created_at as event_time,
    'doubt_modified'::text as event_type
  FROM "doubt_history" dh
  INNER JOIN "doubts" d ON dh.doubt_id = d.id
),
